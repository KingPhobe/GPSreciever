"""Rcubed CONOPS scenario: nominal -> spoofing -> detect invalid -> exit -> reacquire."""
...
from gnss_twin.attacks.spoofing import SpoofPrRampAttack
...
SPOOF_RAMP_RATE_MPS = 20.0
SPOOF_TARGET_SV = "G01"
SPOOF_INVALID_AFTER_S = 8.0
PNT_CFG = PntConfig(
    tta_s=5.0,
    suspect_hold_s=2.0,
    reacq_confirm_s=3.0,
...
def run_scenario() -> dict[str, float | int | None]:
    ...
    attack = SpoofPrRampAttack(start_t=ATTACK_START_S, ramp_rate_mps=SPOOF_RAMP_RATE_MPS, target_sv=SPOOF_TARGET_SV)
    ...
    for t_s in np.arange(0.0, TOTAL_DURATION_S, DT_S):
        ...
        spoof_bias_active = attack_active and (t_s - ATTACK_START_S) >= SPOOF_INVALID_AFTER_S
        if spoof_bias_active:
            integrity = replace(
                integrity,
                is_suspect=True,
                is_invalid=True,
                reason_codes=list(integrity.reason_codes) + ["spoof_bias_detected"],
            )
    ...
    print("=== Scenario: spoof_then_reacquire ===")
